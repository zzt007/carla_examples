...
def simulation(args,world):
    ...
    elif args.batch == True and args.scenario == '4':
        '''
        batch process the scenario 4
        ## change the lane direction, left or right half and half
        ## change the cut in distance, uniform sampling from 10\15\20\25
        ## TODO:change the initial offset of the queue vehicle 1 and 2
        '''
        LANE_CHANGE_DIRECTION = args.lane_change_direction
        CUT_IN_DISTANCE = args.cut_in_distance
        # map the lane change direction from 0 or 1 to left or right
        if LANE_CHANGE_DIRECTION == '0':
            LANE_CHANGE_DIRECTION = 'left'
        elif LANE_CHANGE_DIRECTION == '1':
            LANE_CHANGE_DIRECTION = 'right'
        print('Lane Change Direction: ', LANE_CHANGE_DIRECTION)
        try:
            IS_USE_SPECIFIC_SPAWN_POINT = False
            if  IS_USE_SPECIFIC_SPAWN_POINT:
                # if use the specific spawn point to set up the queue vehicle1 
            ...
            if LANE_CHANGE_DIRECTION == 'left':
                # set up the vehicle2 and 3 at the left lane
                offset_1 = carla.Location(x=20,y=-3.5,z=0) 
                queue_vehicle2_spawn_point = spawn_sur_vehicle_by_offset(queue_vehicle1_spawn_point,offset_1)
                queue_vehicle2 = spawn_vehicle_by_point(world,
                                                        queue_vehicle2_spawn_point,
                                                        '0,0,0')
...
def main():
    ...
    argparser.add_argument(
        '-b','--batch',
        action ='store_true',
        help = 'batch process the simulation'
    )
    argparser.add_argument(
        '-l','--lane_change_direction',
        choices=['0','1'],
        help = 'pick which lane to change, left or right',
        type=str
    )
    argparser.add_argument(
        '-c','--cut_in_distance',
        default = 15,
        help = 'pick the cut in distance',
        type=float
    )
    
    args = argparser.parse_args()
    print('- now batch mode : ',args.batch)
