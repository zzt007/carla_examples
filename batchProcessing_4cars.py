...
def spawn_sur_vehicle_by_relative(ego_spawn_point, rel_distance) -> carla.Transform:
    '''
...
try:
    ...
    sur_spawn_point_1 = spawn_sur_vehicle_by_relative(ego_spawn_point,INITIAL_REL_DISTANCE)
    sur_vehicle_1 = spawn_vehicle_by_point(sur_spawn_point_1, '0,0,0') # 黑色周车
    
    sur_offset_2 = carla.Location(x=-15, y=0, z=0)
    sur_spawn_point_2 = spawn_sur_vehicle_by_offset(ego_spawn_point,sur_offset_2)
    sur_vehicle_2 = spawn_vehicle_by_point(sur_spawn_point_2, '0,0,255') # 蓝色周车
    
    sur_offset_3 = carla.Location(x=-20, y=0, z=0)
    sur_spawn_point_3 = spawn_sur_vehicle_by_offset(ego_spawn_point, sur_offset_3)
    sur_vehicle_3 = spawn_vehicle_by_point(sur_spawn_point_3 , '255,0,0') # 红色周车
    ...
    while not IS_ARRIVE_DESTINATION:
        ...
        sur_vehicle1_speed = carla.Vector3D(x=SUR_VEHICLE_SPEED_LON,
                                           y=SUR_VEHICLE_SPEED_LAT,
                                           z=0)
        
        sur_vehicle2_speed = carla.Vector3D(x=SUR_VEHICLE_SPEED_LON,
                                           y=SUR_VEHICLE_SPEED_LAT,
                                           z=0)

        sur_vehicle3_speed = carla.Vector3D(x=SUR_VEHICLE_SPEED_LON,
                                    y=SUR_VEHICLE_SPEED_LAT,
                                    z=0)

        # 单纯使用waypoints来检测自车是否达到目的地
        ego_vehicle.set_target_velocity(ego_vehicle_speed)
        sur_vehicle_1.set_target_velocity(sur_vehicle1_speed)
        sur_vehicle_2.set_autopilot(True)
        sur_vehicle_3.set_autopilot(True)
        
        if ego_vehicle.get_location().distance(DESTINATION.location) < 10.0:
            IS_ARRIVE_DESTINATION = True
            print('- arrive the destination,end of the simulation')
            
    ego_vehicle.destroy()
    sur_vehicle_1.destroy()
