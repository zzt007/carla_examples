...
def parser():
    ...
    argparser.add_argument(
        ...
        default=10,
        type=int,
        help='Number of background vehicles,depend on the scenario spawn_points num'
    )
    argparser.add_argument(
        '--tm-port',
        metavar='P',
...
def simulation(args,world,client):
    map = world.get_map()
    synchronous_master = False
    spawn_points = map.get_spawn_points()
    origin_settings = world.get_settings() # in order to get the original world setting when turn down the sync mode
    
    vehicle_id_list=[]
    
    if args.scenario == '3':
    # use TrafficManager to spawn the background vehicles
        traffic_manager = client.get_trafficmanager(args.tm_port)
        # set every background vehicle keeps a distance of 5.0 meter      
        traffic_manager.set_global_distance_to_leading_vehicle(5.0)
        # set physical mode
        traffic_manager.set_hybrid_physics_mode(True)
        # set the desired speed, default is 30km/h
        traffic_manager.global_percentage_speed_difference(-20) # negative means accelerate
        ...
        try:
            ...
            # set up the background vehicles' blueprint
            blueprints_background_vehicle = world.get_blueprint_library().filter("vehicle.*")
            # sort the vehicle list by id
            blueprints_background_vehicle = sorted(blueprints_background_vehicle, key=lambda bp: bp.id)
            # make sure the spawn points num is enough
            number_of_spawn_points = len(spawn_points)
            if args.number_of_vehicles < number_of_spawn_points:
                random.shuffle(spawn_points)
            elif args.number_of_vehicles >= number_of_spawn_points:
                msg = 'requested %d vehicles, but could only find %d spawn points'
                print(msg, args.number_of_vehicles, number_of_spawn_points)
                args.number_of_vehicles = number_of_spawn_points - 1
            
            # use command to apply actions on batch of background vehicles
            SpawnActor = carla.command.SpawnActor
            SetAutopilot = carla.command.SetAutopilot
            # this is equal to int 0
            FutureActor = carla.command.FutureActor
            
            # set a blank list to store the batch of background vehicles
            batch = []
            
            for n, transform in enumerate(spawn_points):
                if n >= args.number_of_vehicles:
                    break
                blueprint = random.choice(blueprints_background_vehicle)
            ...
            # wait for a tick to ensure client receives the last transform of the background vehicles
            ...
            world.apply_settings(origin_settings) # 最后需要关闭同步模式，否则carla会宕机
...
def main():
    ...
    print('- now the simulation mode is sync ? : ',args.sync)
